...
mod tests {
    ...
    /// TODO: use openssl
    fn make_test_cert2(subject_alt_names: Vec<String>) -> (rcgen::Certificate, rcgen::KeyPair) {
        use rcgen::{
            date_time_ymd, CertificateParams, DistinguishedName, DnType, KeyPair, SanType,
        };
        ...
        params.subject_alt_names = subject_alt_names
            .iter()
            .map(|s| SanType::DnsName(s.as_str().try_into().unwrap()))
            .collect();
        let key_pair = KeyPair::generate_for(&rcgen::PKCS_RSA_SHA512).unwrap();
        let cert = params.self_signed(&key_pair).unwrap();
        println!("debug: {}", key_pair.serialize_pem());
        (cert, key_pair)
    }
    ...
    fn make_test_cert_ntls(
        ...
        let (cert, key_pair) = make_test_cert2(subject_alt_names);
        let cert2 = native_tls::Certificate::from_der(cert.der()).unwrap();
        //let pkcs8 =
    ...
    async fn basic_ntls() {
        ...
        // send a request with a wrong cert and verify it fails
        {
            let _e = connect_ntls_tonic_channel(&cert2, addr)
                .await
                .expect_err("unexpected success");
            // // there is a double wrappring of the error of ssl Error
            // let src = e.source().unwrap().source().unwrap();
